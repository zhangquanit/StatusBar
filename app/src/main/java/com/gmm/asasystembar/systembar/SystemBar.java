...
public class SystemBar {
    ...
    private void processLollipopUp() {
         ...
         *  状态栏背景:statusBarDrawable 优先 statusBarColor
         *
         */
        if (addStatusBarView && isUsePrivateApi == false) {
            setupStatusBarView(window.getContext(), (ViewGroup) window.getDecorView());
     ...
     */
    private boolean processMIUI(boolean lightStatusBar) throws Exception {
     ...
     */
    private boolean processFlyMe(boolean isLightStatusBar) throws Exception {
        boolean result = false;
        if (window != null) {
            try {
